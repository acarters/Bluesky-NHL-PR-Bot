...
export default class Bot 
{
  ...
  */
  ...
  ) 
  {
    ...
    if (card != "None" && urls[0] == "None") // Check if there is a link card, and there's not at least one url. This might be implemented incorrectly.
    {
      ...
      if (cards[3] != "None")
      {
        ...
        if ((cardBuffer != undefined && cardBuffer.length > 1000000) || cards[3] == "None")
        {
          ...
          var cardResponse = await axios.get("https://assets-cms.thescore.com/uploads/image/file/464215/w640xh480_GettyImages-1231492986.jpg?ts=1626186072", { responseType: 'arraybuffer'}); 
          cardBuffer = Buffer.from(cardResponse.data, "utf-8");
        }
    ...
    var bskyFeedAwait = await this.#agent.getAuthorFeed({actor: "notprnhl.bsky.social", limit: postNum,}); // Get a defined number + 2 of most recent posts from the logged in user's feed.
